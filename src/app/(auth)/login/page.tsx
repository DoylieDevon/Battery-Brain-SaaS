...
function HumLogo() {
  return (
    <div className="flex flex-col items-center mb-8">
      <div style={{ width: 72, height: 72, borderRadius: 22, background: "rgba(14,156,122,.16)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 22 }}>
      ...
      <div style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: 34, letterSpacing: "-.02em", color: "#0E2A24" }}>Hum</div>
      <p className="text-sm text-[#7C8A83] mt-2">Your home battery, on autopilot.</p>
    </div>
  );
}
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-5" style={{ background: "#F7FAF8" }}>
      <div className="w-full max-w-sm">
        <HumLogo />

        <div className="bg-white rounded-3xl p-7 shadow-xl">
          <h2 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 22, color: "#0E2A24", marginBottom: 4 }}>Welcome back</h2>
          <p className="text-sm text-[#7C8A83] mb-6">
            {step === "email" ? "Sign in to check on today's plan." : `We sent a code to ${email}.`}
          </p>
          ...
          {step === "email" ? (
            <>
              <form onSubmit={sendPin} className="space-y-4">
                ...
                <GreenButton type="submit" disabled={loading}>{loading ? "Sending…" : "Send code"}</GreenButton>
              </form>
            ...
            <form onSubmit={verifyPin} className="space-y-4">
              ...
              {error && <p className="text-sm text-red-600">{error}</p>}
              <GreenButton type="submit" disabled={loading}>{loading ? "Verifying…" : "Log in"}</GreenButton>
          ...
          <p className="text-center text-sm text-[#7C8A83] mt-6">
            New here? <Link href="/signup" className="text-[#0E9C7A] font-semibold hover:underline">Connect your battery</Link>
          </p>
        </div>
      </div>
    </div>
